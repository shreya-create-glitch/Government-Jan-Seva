...
{(item.image || item.imageUrl) && (
  <img
    src={
      item.imageUrl
        ? item.imageUrl
        : `http://localhost:1000/${item.image}`
    }
    alt="complaint"
    className="w-full h-48 object-cover rounded-t-2xl"
  />
            ...
            <div className="p-4 flex flex-col flex-grow space-y-2">
              ...
              <div className="flex flex-col gap-2 mt-4">
                {userrole === "admin" && (
                  <button
                    className="bg-blue-500 hover:bg-blue-600 text-white text-sm px-4 py-1.5 rounded-full"
                    onClick={() => handleEdit(item._id)}
                  >
                    ✏️ Edit Status
                  </button>
                ...
                <button
                  className="bg-green-500 hover:bg-green-600 text-white text-sm px-4 py-1.5 rounded-full"
                  onClick={() => handleShare(item)}
                >
                  🔗 Share
                </button>
                ...
                {userId !== item.user?._id && (
                  <button
                    className="bg-purple-500 hover:bg-purple-600 text-white text-sm px-4 py-1.5 rounded-full"
                    onClick={() => handleIncreaseCount(item._id)}
                  >
                    👍 Upvote ({item.count || 0})
                  </button>
